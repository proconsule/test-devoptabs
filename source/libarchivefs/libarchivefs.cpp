...
int CARCHFS::connect(){
	...
	while (true) {
		...
		if(archive_read_next_header(arch_ctx, &entry) != ARCHIVE_OK)break;
		...
		//archentry.st = (struct stat *)malloc(sizeof(struct stat));
		//archentry.st = (struct stat *)archive_entry_stat(entry);
